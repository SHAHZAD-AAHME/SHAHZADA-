...
"""%(N))                                                    
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Admin       : SAJJAD VERSION 2.0
 [\x1b[1;96m+%s] WhatsApp   : +8801840215106
 [\x1b[1;96m+%s] Facebook   : Shahzada Ahmed Sajjad 
 [\x1b[1;96m+%s] Github      : https://github.com/SHAHZADA-AHMED
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
def kontol():
    os.system("clear")
    print(f""" 
...
{P}[•]{B}----------------------------------------------------{P}[•]
{B} |
{P}[•] ADMKN LISESI : SAJJAD
{P}[•] WHATSAPP : +8801840215106
{P}[•] FACEBOOK : Shahzada Ahmed Sajjad""")
def licensi():#line:42
  try :#line:43
    os .system ('clear')
    kontol()
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [SAJJAD-XD] %s/%s •_• [OK] %s •_• [CP] %s •_• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
